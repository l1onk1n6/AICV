...
//
// Authentifizierung passiert IM Code (admin.auth.getUser), deshalb laeuft das
// Gateway bewusst ohne eigene JWT-Pruefung.
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (authError || !authUser) {
      ...
      return json({
        error: 'Ungültige Sitzung. Bitte neu anmelden.',
        step: 'auth'
      }, 401);
    }
    ...
    const { count, error: countErr } = await admin.from('contact_log').select('id', {
      count: 'exact',
      head: true
    ...
    const articleBody = [
      ...
      customerEmail ? `<b>E-Mail:</b> ${esc(customerEmail)}` : '',
    ...
    if (!zammadRes.ok) {
      ...
      return json({
        error: `Zammad ${zammadRes.status}: ${errText}`,
        step: 'sending'
      }, 500);
    }
...
<body style="margin:0;padding:0;background:#0a0a0a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
  ...
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#0a0a0a;padding:48px 16px;">
    ...
    _step = 'log';
    const { error: logErr } = await admin.from('contact_log').insert({
      user_id: userId
    });
